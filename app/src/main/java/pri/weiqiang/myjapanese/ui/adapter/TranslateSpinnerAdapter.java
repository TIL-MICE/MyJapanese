...
public class TranslateSpinnerAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (spinnerItem.isHasIcon()) {
            view = LayoutInflater.from(context).inflate(pri.weiqiang.myjapanese.R.layout.spinner_item_translate_icon, parent, false);
            TextView textView = (TextView) view.findViewById(pri.weiqiang.myjapanese.R.id.tv_name);
            textView.setText(spinnerItem.getName());
            ImageView imageView = (ImageView) view.findViewById(pri.weiqiang.myjapanese.R.id.iv_icon);
            Glide.with(context)/*.asBitmap()*/.load(spinnerItem.getIcon()).into(imageView);

        } else {
            view = LayoutInflater.from(context).inflate(pri.weiqiang.myjapanese.R.layout.spinner_item_translate, parent, false);
            TextView textView = (TextView) view.findViewById(pri.weiqiang.myjapanese.R.id.tv_name);
            textView.setText(spinnerItem.getName());
        }
